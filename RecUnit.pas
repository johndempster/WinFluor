...
begin
    ...
    if MainFrm.EXCSingleWaveLength then
     ...
     NumFramesInBuffer := Min( NumFramesInBuffer, Round(8.0/edFrameInterval.Value) ) ;
...
begin
    ...
    Wait(0.1) ;
     ...
     NumDACPointsPerFrame := Round(edFrameInterval.Value/DACUpdateInterval) ;
     edFrameInterval.Value := NumDACPointsPerFrame*DACUpdateInterval ;
     ...
     if (cbRecordingMode.ItemIndex = rmTimeLapse) or (cbRecordingMode.ItemIndex = rmTimeLapseBurst)then begin
        NumFramesPerTimeLapseInterval := Max(Round(EdTimeLapseInterval.Value/edFrameInterval.Value),1) ;
        ...
        EdTimeLapseInterval.Value := NumFramesPerTimeLapseInterval*edFrameInterval.Value ;
        TimeLapseFrameInterval := GetTimeLapseFrameInterval ;
        end ;

     // Total number of DAC updates in timing cycle
     NumFramesPerCycle := Max(Round(5.0/edFrameInterval.Value) div NumFramesPerCycle,1)*NumFramesPerCycle ;
...
var
     ...
     ExcitationOffAtFrame,ExcitationOnAtFrame,ExcitationFrame : Integer ;
...
begin
     ...
     if (cbRecordingMode.ItemIndex <> rmContinuous) and
        ...
        (not BurstIlluminationOn) then begin
          ...
          ExcitationOffAtFrame := NumFramesPerWavelengthCycle ;
          ExcitationOnAtFrame := NumFramesPerTimeLapseInterval ;
          end
     else begin
          // continuous recording - excitation on for all frames
          ExcitationOffAtFrame := NumFramesPerCycle ;
          ExcitationOnAtFrame := NumFramesPerCycle ;
          end ;
     if not rbEXCShutterOpen.Checked then begin
          // Shutter closed - excitation off for all frames
          ExcitationOffAtFrame := 0 ;
          ExcitationOnAtFrame := 0 ;
          end ;
     ...
     for iFrame := 0 to NumFramesPerCycle-1 do begin
         ...
         if (ExcitationFrame >= ExcitationOffAtFrame) then begin
            // Get voltages for shutters closed condition
         ...
         Inc(ExcitationFrame) ;
         if ExcitationFrame >= ExcitationOnAtFrame then ExcitationFrame := 0 ;
...
begin
     ...
     if (cbRecordingMode.ItemIndex = rmTimeLapse)  and (RecordingMode = rmRecordingInProgress) then
        ExcitationOffAtFrame := NumWavelengths
     else ExcitationOffAtFrame := NumFramesPerCycle ;
     if not rbEXCShutterOpen.Checked then ExcitationOffAtFrame := 0 ;
     ...
     for iFrame := 0 to NumFramesPerCycle-1 do begin
         ...
         if iFrame >= ExcitationOffAtFrame then begin
            // Shutter closed
...
begin
    ...
    StartIllumination := StartBurstAtFrame - Round(BurstIlluminationDelay/edFrameInterval.Value) ;
              ...
              if cbRecordingMode.ItemIndex = rmContinuous then begin
                 ...
                 if (cbRecordingMode.ItemIndex = rmTimelapseBurst) and
                    ...
                    if NumBurstFramesDone >= NumBurstFramesRequired then begin
                       ...
                       NumBurstFramesRequired := Max( Round(edBurstDuration.Value/edFrameInterval.Value),
                                                      NumFrameTypes) ;
                       StartBurstAtFrame := StartBurstAtFrame +
                                            Max( Round(edBurstInterval.Value/edFrameInterval.Value),
...
begin
     ...
     if cbRecordingMode.ItemIndex = rmContinuous then begin
        ...
        if cbRecordingMode.ItemIndex = rmTimelapseBurst then begin
           ...
           BurstCounter := 0 ;
           NumBurstFramesDone := 0 ;
           NumBurstFramesRequired := Max(Round(edBurstDuration.Value/edFrameInterval.Value),NumFrameTypes) ;
           StartBurstAtFrame := Max(Round(edBurstInterval.Value/edFrameInterval.Value),1) ;
           IDRFileBurst.Ident := MainFrm.IDRFile.Ident +
                                 format('Burst T=%.5gs Frame=%d',[StartBurstAtFrame*IDRFileBurst.FrameInterval,StartBurstAtFrame]);
...
begin
     ...
     if IDRFileBurst.NumFrames < 1 then DeleteFile( PANSIchar(IDRFileBurst.FileName)) ;
