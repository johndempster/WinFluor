...
uses
  ...
  ValidatedEdit, IDRFile, ClipBrd, Printers, HTMLLabel, math, strutils ;
...
begin
    ...
    if FileRead( FileHandle, ROIs, SizeOf(ROIs)) <> SizeOf(ROIs) then
...
begin
     ...
     if FileHandle < 0 then begin
        MainFrm.StatusBar.SimpleText := 'SAVE ROI: Unable to save ' + FileName ;
        Exit ;
        end ;

     for i := 0 to MainFrm.IDRFile.MaxROI do begin
         ROIs[i] := MainFrm.IDRFile.ROI[i] ;
         end ;

     // Save ROI record
     if FileWrite( FileHandle, ROIs, SizeOf(ROIs)) <> SizeOf(ROIs) then
        MainFrm.StatusBar.SimpleText := 'SAVE ROI: Error saving ' + FileName ;
